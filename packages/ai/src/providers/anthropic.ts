...
/**
 * Hard API limit. Anthropic rejects any image whose width or height exceeds
 * this ("At least one of the image dimensions exceed max allowed size: 8000
 * pixels") no matter how many images the request carries, and the rejection
 * poisons every retry and model fallback because the block stays in context.
 * Tall full-page screenshots cross it routinely, so every request clamps to it.
 */
const ANTHROPIC_MAX_IMAGE_DIMENSION = 8000;
type AnthropicImageCap = typeof ANTHROPIC_MANY_IMAGE_MAX_DIMENSION | typeof ANTHROPIC_MAX_IMAGE_DIMENSION;
 ...
 * depends on image count, so each cap keeps its own cache: a value cached at
 * 8000px is not reusable at 2000px.
 */
const anthropicImageResizeCaches: Record<AnthropicImageCap, WeakMap<ImageContent, ImageContent>> = {
	[ANTHROPIC_MANY_IMAGE_MAX_DIMENSION]: new WeakMap(),
	[ANTHROPIC_MAX_IMAGE_DIMENSION]: new WeakMap(),
};
...
async function resizeAnthropicImageContent(
	...
	limit: ResizeLimiter,
	maxDimension: AnthropicImageCap,
	...
	const next = await Promise.all(
		content.map(async block => {
			...
			if (
				...
				return block;
			const cache = anthropicImageResizeCaches[maxDimension];
...
async function prepareAnthropicImageContext(context: Context, supportsImages: boolean): Promise<Context> {
	...
	// A smaller set only has to clear Anthropic's hard per-image limit.
	const maxDimension =
		imageCount > ANTHROPIC_MANY_IMAGE_THRESHOLD ? ANTHROPIC_MANY_IMAGE_MAX_DIMENSION : ANTHROPIC_MAX_IMAGE_DIMENSION;
...
const streamAnthropicOnce = (
	...
	(async () => {
		...
		try {
			...
			const preparedContext = await prepareAnthropicImageContext(context, model.input.includes("image"));
